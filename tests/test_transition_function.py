...
def test_transition_function_takes_move():
    ...
    env.reset()
    state = env.env.go_game

    new_state = transition_function(state, action=0)
    assert not np.array_equal(state.board, new_state.board)
...
def test_transition_function_correct_move():
    ...
    starting_state: Position = env.env.go_game
    starting_board = starting_state.board
    assert np.all(starting_board == 0)

    player_int = -1 if env.player == "white_0" else 1
    ...
    assert new_board[0, 0] == player_int

    np.testing.assert_array_equal(env.env.go_game.board, new_board)
